...
<head>
    ...
    <title>@yield('title', 'QCC Union Dashboard') - Admin</title>
    ...
    <aside class="w-64 glass-sidebar flex flex-col min-h-screen text-slate-300 shrink-0">
        ...
        <div class="h-16 flex items-center px-6 border-b border-slate-800">
            <div class="flex items-center space-x-3">
                <div class="w-8 h-8 rounded-lg bg-[#D70321] flex items-center justify-center font-bold text-white text-lg shadow-md">U</div>
                <span class="font-bold text-white text-lg tracking-wide uppercase">QCC UNION</span>
            </div>
        </div>
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
            ...
            <div class="pt-4 pb-2 px-4 uppercase text-xs font-semibold text-slate-500 tracking-wider">Pledge Day</div>

            <a href="{{ route('pledges.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-slate-800 hover:text-white transition duration-200 {{ request()->is('pledges*') ? 'nav-item-active text-white' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                <span>Pledges</span>
                @php $pledgeCount = \App\Models\PledgeRecord::count(); @endphp
                @if($pledgeCount > 0)
                    <span class="ml-auto bg-emerald-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ number_format($pledgeCount) }}</span>
                @endif
            </a>
        </nav>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm shrink-0">
            ...
            <div class="flex items-center space-x-4">
                <span class="text-sm text-slate-500 font-semibold">QCC Union Day Kiosk</span>
            </div>
        </header>
